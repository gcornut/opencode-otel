...
describe("claude-code telemetry profile", () => {
  ...
  test("tool_result.success is stringified in claude-code profile", () => {
    const h = setup({
      telemetryProfile: "claude-code",
      logToolDetails: true,
    })
    ...
    h.emit({
      type: "tool.execute.before",
      properties: { tool: "read", sessionID: "s1", callID: "c1", args: {} },
    })
    h.emit({
      type: "tool.execute.after",
      properties: { tool: "read", sessionID: "s1", callID: "c1", args: {}, output: "file contents" },
    })

    const events = h.getEventsByName("tool_result")
    expect(events.length).toBe(1)
    // success should be a string ("true"/"false"), not a boolean
    expect(typeof events[0].attributes["success"]).toBe("string")
    expect(events[0].attributes["success"]).toBe("true")
  })

  test("tool_result.success is boolean in default profile", () => {
    const h = setup({ logToolDetails: true })
    ...
    const events = h.getEventsByName("tool_result")
    expect(events.length).toBe(1)
    // In default (opencode) profile, success stays a boolean
    expect(typeof events[0].attributes["success"]).toBe("boolean")
    expect(events[0].attributes["success"]).toBe(true)
  })

  test("tool_name is PascalCase in claude-code profile", () => {
    const h = setup({
      telemetryProfile: "claude-code",
      logToolDetails: true,
    })
    ...
    for (const tool of ["read", "bash", "glob", "write", "edit"]) {
      h.emit({
        type: "tool.execute.before",
        properties: { tool, sessionID: "s1", callID: `c-${tool}`, args: {} },
      })
      h.emit({
        type: "tool.execute.after",
        properties: { tool, sessionID: "s1", callID: `c-${tool}`, args: {}, output: "ok" },
      })
    }

    const events = h.getEventsByName("tool_result")
    expect(events.length).toBe(5)
    expect(events[0].attributes["tool_name"]).toBe("Read")
    expect(events[1].attributes["tool_name"]).toBe("Bash")
    expect(events[2].attributes["tool_name"]).toBe("Glob")
    expect(events[3].attributes["tool_name"]).toBe("Write")
    expect(events[4].attributes["tool_name"]).toBe("Edit")
  })

  test("tool_name is lowercase in default profile", () => {
    const h = setup({ logToolDetails: true })
    ...
    h.emit({
      type: "tool.execute.after",
      properties: { tool: "read", sessionID: "s1", callID: "c1", args: {}, output: "ok" },
    })
