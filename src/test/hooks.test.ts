...
describe("token and cost tracking", () => {
  test("step-finish with usage emits token metrics and api_request event", async () => {
    const h = setup()
  ...
  test("step-start with usage also emits metrics", async () => {
    const h = setup()
    ...
    const tokenMetrics = h.getMetricsByName("opencode.token.usage")
    expect(tokenMetrics.length).toBeGreaterThanOrEqual(2)

    const input = tokenMetrics.find((m) => m.attributes.type === "input")
...
describe("tool execution", () => {
  test("tool.execute.before records pending call and emits tool.decision metric", async () => {
    const h = setup()
    ...
    expect(metrics.length).toBe(1)
    expect(metrics[0].attributes.tool_name).toBe("bash")
    expect(metrics[0].attributes.decision).toBe("execute")
  })
...
describe("prompt.id propagation", () => {
  test("prompt.id from chat.message propagates to subsequent api_request events", () => {
    ...
    // Subsequent api_request should carry the same promptId
    h.emit({
      type: "message.part.updated",
      properties: {
        part: {
...
describe("claude-code telemetry profile", () => {
  ...
  test("uses claude_code event prefix in log body", () => {
    ...
    // event.name is always unprefixed
    const events = h.getEventsByName("session.created")
    expect(events.length).toBe(1)

    // log body uses the prefixed form
    expect(events[0].body).toBe("claude_code.session.created")
  })
  ...
  test("all 8 metric names match Claude Code naming", async () => {
    ...
    // Tool decision + tool result + commit + PR
    h.emit({
      type: "tool.execute.before",
      properties: {
        tool: "bash",
